...
#-------------------------------------------------------------------------------
simulate_MIMOSA2_alt_prior <- function(
  effect = 5e-4,
  bg_effect = 0,
  ...
  if (effect < 0) {
    stop("'effect' must be nonnegative.")
  }
  if (baseline_stim_effect < 0) {
    stop("'baseline_stim_effect' must be nonnegative.")
  }
  if (bg_effect < 0) {
    stop("'bg_effect' must be nonnegative.")
  }

  K <- 8
  n <- rep(0, K)

  while (sum(n) != P || any(n == 0)) {
  ...
  is_odds_gamma <- prior %in%
    c("Odds_gamma", "Odds gamma", "odds_gamma", "odds gamma", "Og", "OG", "og")
  is_unit_lognormal <- prior %in%
    c(
      "Unit_lognormal",
      "Unit lognormal",
      "unit_lognormal",
  ...
  rgamma_exp <- function(num_draws, target_mean, gamma_dispersion) {
    ...
    outs <- exp(
      -rgamma(num_draws, gamma_dispersion, gamma_dispersion / gamma_mean)
    )
    return(outs)
  }
  ...
  rsimplex <- function(num_draws, target_mean, component_phi) {
    ...
    v <- rnorm(num_draws)^2
    x <- mu_ig +
      (mu_ig^2 * v) / (2 * lambda_ig) -
      (mu_ig / (2 * lambda_ig)) *
        sqrt(4 * mu_ig * lambda_ig * v + mu_ig^2 * v^2)
    z <- runif(num_draws)
  ...
  if (is_unif) {
    ...
    if (any(bad)) {
      ...
      stop(paste(
        "Precisions too small for a Uniform prior.",
        "Required minimum precisions are:",
        msg,
        sep = "\n"
      ))
    }
