...
#-------------------------------------------------------------------------------
# 1. Update parameters: Anchor u0 and u1 at the same center (0.0010)
# to clearly isolate the shift in stimulation (s0 vs s1)
params <- data.frame(
  Timepoint = factor(c("Timepoint 0", "Timepoint 0", 
                       "Timepoint 1", "Timepoint 1"),
                     levels = c("Timepoint 0", "Timepoint 1")),
  Condition = factor(c("u0", "s0", "u1", "s1"), levels = c("u0", "s0", "u1", "s1")),
  Label     = c("Unstimulated", "Stimulated", "Unstimulated", "Stimulated"),
  mean      = c(0.0010, 0.0018, 0.0010, 0.0035), 
  sd        = c(0.0005, 0.0005, 0.0005, 0.0005)
)

x_grid <- seq(-0.001, 0.006, length.out = 1000)

plot_data <- params |>
  group_by(Timepoint, Condition, Label) |>
  reframe(
    x = x_grid,
    density = dnorm(x_grid, mean = mean, sd = sd)
  )

# Annotation metadata for arrows inside facets
arrows_data <- data.frame(
  Timepoint = factor(c("Timepoint 0", "Timepoint 1"), 
                     levels = c("Timepoint 0", "Timepoint 1")),
  x_start   = c(0.0010, 0.0010),
  x_end     = c(0.0018, 0.0035),
  y         = c(450, 450),
  label     = c("s0 - u0", "s1 - u1")
)

# 2. Plot stacked densities
vaccine_effect_plot <- ggplot(plot_data, aes(x = x, y = density, fill = Label, color = Label)) +
  geom_area(position = "identity", alpha = 0.4) +
  geom_line(linewidth = 0.9) +
  geom_vline(data = params, aes(xintercept = mean, color = Label), 
             linetype = "dashed", linewidth = 0.6, show.legend = FALSE) +
  
  # Add DiD distance arrows within each stacked panel
  ...
  geom_text(data = arrows_data, 
            aes(x = (x_start + x_end) / 2, y = y + 120, label = label),
            inherit.aes = FALSE, fontface = "bold", size = 4, color = "grey20") +
  
  # Stack vertically into 2 panels
  facet_wrap(~ Timepoint, ncol = 1, scales = "free_y") +
  
  scale_fill_manual(values = c("Unstimulated" = "orchid", "Stimulated" = "darkgreen")) +
  scale_color_manual(values = c("Unstimulated" = "orchid", "Stimulated" = "darkgreen")) +
  scale_x_continuous(limits = c(-0.0005, 0.0055), breaks = seq(0, 0.005, 0.001)) +
  scale_y_continuous(expand = expansion(mult = c(0, 0.25))) +
  
  labs(
    title = "MIMOSA2 model framework.",
    subtitle = "Vaccine effect: (s1 - u1) - (s0 - u0).",
    x = "Functional cell proportions",
    y = ""
  ) +
  theme_minimal(base_size = 14) +
...
print(vaccine_effect_plot)
ggsave('_fig/MIMOSA2_fw_stacked.pdf', plot = vaccine_effect_plot, width = 8, height = 6)
